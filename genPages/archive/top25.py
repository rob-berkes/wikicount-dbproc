...
def HTMLHeader(FILE):
	FILE.write("<HTML><HEAD>Top 25 Pages</HEAD>")
...
HTMLFILE=HTMLDIR+"/top25_"+str(YEAR)+"_"+str(MONTH)+"_"+str(DAY)+".html"
conn=Connection()
db=conn.wc
QUERY=db.tophits.find({'d':int(DAY)}).sort('place',1).limit(25)
