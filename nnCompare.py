...
def printDayDifferences():
	...
	try:
		...
		for a in range(1,15):
			print 'Day: '+str(a)+' Diff Change: '+str(float(VALUES[a])-oldVALUES[a])+'    Total: '+str(float(VALUES[a])-initVALUES[a])
	except TypeError:
		print "Old values appear not to exist.  Please rerun after doing some more scoring, it should then work"
