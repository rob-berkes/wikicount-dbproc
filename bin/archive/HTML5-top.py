...
def TABLEBuildTabs(FILE,RECPERTABSET,QUERY):
	FILE.write("<center><h3><center>Top Wikipedia Pages for "+time.strftime("%D")+"</center></h3>")
	FILE.write('<h3>Yesterday\'s '+str(RECPERTABSET)+' most viewed Wikipedia articles.</h3></center>')
...
HTMLFILE=HTMLDIR+"/top_"+str(YEAR)+"_"+str(MONTH)+"_"+str(DAY)+".html"
conn=Connection()
db=conn.wc
